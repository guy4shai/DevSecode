...
function getSeverity(entropy) {
  if (entropy >= 4.8) return { level: 'High', color: 'red' };
  if (entropy >= 4.3) return { level: 'Medium', color: 'orange' };
...
function getSeverityScore(level) {
  return { 'High': 1, 'Medium': 2, 'Low': 3 }[level] || 4;
}
...
function filterFindings(findings, config) {
  const selectedSeverities = config.selectedSeverities || ['High', 'Medium', 'Low'];
...
async function generatePDFReport(gitleaksFindings, config, tools = {}) {
  ...
  doc.font('Courier-Bold').fontSize(20).fillColor('black').text('DevSecode Unified Report', { align: 'center' });
  doc.moveDown();
